...
public class Zadacha2 {
    public static void main(String[] args){
        ...
        int[] sadnwiches = new int[parts.length];
        for(int i=0;i< parts.length;i++){
            sadnwiches[i] = Integer.parseInt(parts[i]);
        }

        ArrayQueue<Integer> students = new ArrayQueue<>(tickets.length);
        for(int i=0;i< tickets.length;i++){
            students.enqueue(tickets[i]);
        }
